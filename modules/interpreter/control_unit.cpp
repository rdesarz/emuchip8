...
void ControlUnit::jumpToLocation(uint16_t address) { m_pc = address; }
...
void ControlUnit::skipNextInstructionIfEqual(uint8_t value, RegisterId reg) {
  if (m_registers[reg] == value) {
    m_pc += 2;
  }
}

void ControlUnit::skipNextInstructionIfNotEqual(uint8_t value, RegisterId reg) {
  if (m_registers[reg] != value) {
    m_pc += 2;
  }
}
...
void ControlUnit::storeInRegister(uint8_t value, RegisterId reg) {
  m_registers[reg] = value;
}

void ControlUnit::addToRegister(uint8_t value, RegisterId reg) {
  m_registers[reg] += value;
}

void ControlUnit::storeRegisterInRegister(RegisterId reg_x, RegisterId reg_y) {
  m_registers[reg_x] = m_registers[reg_y];
}

void ControlUnit::bitwiseOr(RegisterId reg_x, RegisterId reg_y) {
  m_registers[reg_x] = (m_registers[reg_x] | m_registers[reg_y]);
}

void ControlUnit::bitwiseAnd(RegisterId reg_x, RegisterId reg_y) {
  m_registers[reg_x] = (m_registers[reg_x] & m_registers[reg_y]);
}

void ControlUnit::bitwiseXor(RegisterId reg_x, RegisterId reg_y) {
  m_registers[reg_x] = (m_registers[reg_x] ^ m_registers[reg_y]);
}

void ControlUnit::addRegisterToRegister(RegisterId reg_x, RegisterId reg_y) {
...
void ControlUnit::displayOnScreen(uint16_t n_bytes_to_read, RegisterId reg_x,
                                  RegisterId reg_y) {
