...
{
public:
    TestDisplayModel(std::size_t width, std::size_t height) : m_pixels(width,height)
    { 
        m_pixels.clear();
    }

    virtual void setPixelValue(std::size_t x, std::size_t y, uint8_t value) override
    {
        m_pixels(x,y) = value;
    }
...
{
protected:
    TestDisplayFixture() : model(64,32), view(new TestDisplayView), display(model, view) { }
...
{
protected:
    TestControlUnitFixture() : model(64,32), view(new TestDisplayView()), display_ctrler(model, view), ctrl_unit(pc, stack_ptr, mem_add_reg, stack, registers, ram, display_ctrler, ui_ctrler), registers(16)
    { }
